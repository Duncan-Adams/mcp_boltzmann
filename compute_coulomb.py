...
def compute_coulomb_rate(mass, Tm_min, Tm_max, n_temps, outdir, overwrite, n_strat, neval, nitn, nproc):
    warnings.filterwarnings('ignore')
    
    outfile_name = f'mcp_coulomb_rate_m_{mass}_Q_1.npz'
    outfile_path = os.path.join(outdir,outfile_name)
    
    if (overwrite is False) and os.path.exists(outfile_path):
    ...
    for i in range(len(temp_grid_2d)):
        ...
        result_e[i] = rate_int_e.compute_QS(T_sm, T_ds, n_strat=n_strat, neval=neval, nitn=nitn, nproc=nproc)[0]
        result_mu[i] = rate_int_mu.compute_QS(T_sm, T_ds, n_strat=n_strat, neval=neval, nitn=nitn, nproc=nproc)[0]
        result_tau[i] = rate_int_tau.compute_QS(T_sm, T_ds, n_strat=n_strat, neval=neval, nitn=nitn, nproc=nproc)[0]
        
        if T_sm > LQCD:
            result_lq[i] = rate_int_lq.compute_QS(T_sm, T_ds, n_strat=n_strat, neval=neval, nitn=nitn, nproc=nproc)[0]
            result_strange[i] = rate_int_strange.compute_QS(T_sm, T_ds, n_strat=n_strat, neval=neval, nitn=nitn, nproc=nproc)[0]
            result_charm[i] = rate_int_charm.compute_QS(T_sm, T_ds, n_strat=n_strat, neval=neval, nitn=nitn, nproc=nproc)[0]
    ...
    #save to a npz file
    np.savez_compressed(outfile_path, n_temps = n_temps, temp_grid = temp_grid_2d, rate = result_total)  
