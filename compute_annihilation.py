...
def compute_annihilation_rate_mcp_fermionic(task, n_temps, outdir, maxwell_boltzmann=False, summed=True):
    ...
    sigma_uu_ff_Z = partial(ann.sigma_ff_xx_fermionic_Z_boson, m_mcp = m_mcp, m_f = 0.0, q_f=q_u, cv=ann.cv_u, ca=ann.ca_u)
    sigma_dd_ff_Z = partial(ann.sigma_ff_xx_fermionic_Z_boson, m_mcp = m_mcp, m_f = 0.0, q_f=q_d, cv=ann.cv_d, ca=ann.ca_d)
    sigma_ss_ff_Z = partial(ann.sigma_ff_xx_fermionic_Z_boson, m_mcp = m_mcp, m_f = m_s, q_f=q_s, cv=ann.cv_s, ca=ann.ca_s)
    sigma_cc_ff_Z = partial(ann.sigma_ff_xx_fermionic_Z_boson, m_mcp = m_mcp, m_f = m_c, q_f=q_c, cv=ann.cv_c, ca=ann.ca_c)
    sigma_bb_ff_Z = partial(ann.sigma_ff_xx_fermionic_Z_boson, m_mcp = m_mcp, m_f = m_b, q_f=q_b, cv=ann.cv_b, ca=ann.ca_b)
    sigma_tt_ff_Z = partial(ann.sigma_ff_xx_fermionic_Z_boson, m_mcp = m_mcp, m_f = m_t, q_f=q_t, cv=ann.cv_t, ca=ann.ca_t)
    ...
    if maxwell_boltzmann is True:
        ...
        Ix_table_lq = np.array(
            [ann.Ix(sigma_lq_ff_Z, m_mcp, 0.0, T) for T in Temp_grid]
        )
        ...
        Ix_table_pipi = np.array(
            [ann.Ix(sigma_pipi_ff_Z, m_mcp, m_pi, T) for T in Temp_grid]
        )
        ...
        Ix_table_lq = np.array(
            [ann.Ix_fermi(sigma_lq_ff_Z, m_mcp, 0.0, T) for T in Temp_grid]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--summed', dest='summed', action='store_true', default=True help='output file contains only sum total rate')
    ...
    fun_loop = partial(
        compute_annihilation_rate_mcp_fermionic_gamma_only, 
