...
# ~ FAC = 1./(6.58212e-22)
FAC = 1.0 #we directly work in inverse MeV
...
P_int      =interp1d(np.loadtxt(QED_dir + "QED_P_int.cvs")[:,0],np.loadtxt(QED_dir + "QED_P_int.cvs")[:,1]+np.loadtxt(QED_dir + "QED_P_int.cvs")[:,2],bounds_error=False,fill_value=0.0,kind=_INTERP_METHOD)
dP_intdT   =interp1d(np.loadtxt(QED_dir + "QED_dP_intdT.cvs")[:,0],np.loadtxt(QED_dir + "QED_dP_intdT.cvs")[:,1]+np.loadtxt(QED_dir + "QED_dP_intdT.cvs")[:,2],bounds_error=False,fill_value=0.0,kind=_INTERP_METHOD)
d2P_intdT2 =interp1d(np.loadtxt(QED_dir + "QED_d2P_intdT2.cvs")[:,0],np.loadtxt(QED_dir + "QED_d2P_intdT2.cvs")[:,1]+np.loadtxt(QED_dir + "QED_d2P_intdT2.cvs")[:,2],bounds_error=False,fill_value=0.0,kind=_INTERP_METHOD)
...
f_nue_s  = interp1d(np.loadtxt(rates_dir+"nue_scatt.dat")[:,0],np.loadtxt(rates_dir+"nue_scatt.dat")[:,1],kind=_INTERP_METHOD)
f_numu_s = interp1d(np.loadtxt(rates_dir+"numu_scatt.dat")[:,0],np.loadtxt(rates_dir+"numu_scatt.dat")[:,1],kind=_INTERP_METHOD)
f_nue_a  = interp1d(np.loadtxt(rates_dir+"nue_ann.dat")[:,0],np.loadtxt(rates_dir+"nue_ann.dat")[:,1],kind=_INTERP_METHOD)
f_numu_a = interp1d(np.loadtxt(rates_dir+"numu_ann.dat")[:,0],np.loadtxt(rates_dir+"numu_ann.dat")[:,1],kind=_INTERP_METHOD)
f_nue_scan = interp1d(np.loadtxt(rates_dir+"sigmav_nue.dat")[:,0],np.loadtxt(rates_dir+"sigmav_nue.dat")[:,1],kind=_INTERP_METHOD)
f_numu_scan = interp1d(np.loadtxt(rates_dir+"sigmav_numu.dat")[:,0],np.loadtxt(rates_dir+"sigmav_numu.dat")[:,1],kind=_INTERP_METHOD)
...
def DeltaRho_nue_nochem(T_gam, T_numu, T_nue):
    pref =  FAC*GF**2/np.pi**5

    drho = ( 4* (geL**2 + geR**2) * Ffunc_nue_e(T_gam,T_nue)  + 2*Ffunc(T_numu,T_nue) )

    return pref*drho

def DeltaRho_numu_nochem(T_gam, T_numu, T_nue):
    pref =  FAC*GF**2/np.pi**5

    drho = ( 4* (gmuL**2 + gmuR**2) * Ffunc_numu_e(T_gam,T_numu) - Ffunc(T_numu,T_nue) )
